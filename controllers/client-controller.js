...
exports.postSignUp = [
  ...
  async (req, res, next) => {
    try {
      ...
      let token;
      try {
        token = await fetch(`http://localhost:8001/api/AaaS/token/v1/client`, {
          method: "POST",
          headers: {
        ...
        if (!token.ok) {
          ...
          return res
            .status(token.status)
            .json(new Response(token.status, null, "token service not work"));
        }
      } catch (err) {
        await prisma.Client.delete({
          where: {
        ...
        return res
          .status(500)
          .json({ Message: "Error genrating token from token service", err });
      }
